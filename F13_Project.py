...
def grade(num):
    if num>=90:
        ...
        return("Passed with grade E.")
    else:
        return("Failed. Grade F.")
...
while message:
    ...
    with open(f'{path}/Department.csv','a',newline='')as f:
        ...
        script.writerow(['CSBS', 'Computer Science and business System'])
...
for i in range(student_no):
    ...
    if duplicate('Student.csv',student_id,0):
        ...
        with open(f"{path}/ReportCards/{student_id}_{''.join(name.split())}.txt",'w') as f:
            ...
            f.write('\n')
            f.write(f'You have got {total_marks} in total with {float(total_marks/count(marks_lst))}%\n')
